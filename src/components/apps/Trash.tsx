...
export default function Trash() {
  ...
  const handleDownload = () => {
    ...
    // Your resume in /public
    link.href = "/Mani_Resume.pdf";
  ...
  return (
    <div className="flex flex-col h-full bg-white text-black font-sans">
        ...
        <div className="flex gap-2">
          {!isEmpty && (
            <button
              ...
              disabled={isEmptying}
              className="flex items-center gap-1.5 px-3 py-1 bg-white border border-gray-300 rounded-md text-xs font-medium text-gray-700 hover:bg-gray-50 active:bg-gray-100  shadow-sm disabled:opacity-50 transition-all cursor-pointer active:scale-[0.98] duration-200"
            >
              {isEmptying ? (
          ...
          {isEmpty && (
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-1 bg-white border border-gray-300 rounded-md text-xs font-medium text-blue-600 hover:bg-blue-50  shadow-sm transition-all cursor-pointer active:scale-[0.98] duration-200"
            >
              <RefreshCw size={12} />
              Undo
            </button>
      ...
      <div className="flex-1 p-6 bg-white overflow-auto">
        {isEmpty ? (
          ...
          <div className="space-y-2">
            <div className="text-xs font-medium text-gray-500 mb-4 pl-1">
              1 item selected
            </div>
            ...
            <div className="group flex items-center justify-between p-3 rounded-lg border border-gray-200 hover:border-blue-400 hover:bg-blue-50/30 transition-all cursor-default">
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="flex items-center gap-2 px-4 py-2 bg-blue-600 text-white text-sm font-medium rounded-md hover:bg-blue-700 shadow-sm transition-all hover:-translate-y-0.5  cursor-pointer active:scale-[0.98] duration-200"
                >
                  <Download size={16} />
                  Recover & Download
                </button>
            ...
            <div className="mt-4 flex gap-3 p-4 bg-yellow-50 rounded-lg border border-yellow-100 text-yellow-800 text-sm">
              ...
              <div>
                ...
                <p className="mt-1 text-yellow-700/80">
                  Sometimes great candidates get lost in the pile. Don't let
                  this one go! Recover it now before the system auto-deletes it.
                </p>
              </div>
            </div>
          </div>
      ...
      <div className="h-8 bg-gray-50 border-t border-gray-200 flex items-center px-4 text-xs text-gray-500">
        {!isEmpty ? "1 item, 1 MB available" : "0 items"}
      </div>
    </div>
  );
}
