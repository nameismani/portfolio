...
export default function TopBar() {
  ...
  return (
    <div
      ref={menuRef}
      className="h-8 w-full bg-black/20 backdrop-blur-md flex items-center justify-between px-4 text-white text-sm fixed top-0 left-0 z-[100] select-none shadow-sm transition-all duration-200"
    >
      <div className="flex items-center gap-4">
        {/* Apple Menu */}
        <div className="relative">
          <div
            className={`relative w-4 h-4 cursor-pointer hover:opacity-80 ${
              activeMenu === "apple" ? "opacity-100" : "opacity-90"
            }`}
        ...
        <div className="relative">
          ...
          {activeMenu === "Portfolio" && (
            <Dropdown items={menuItems.Portfolio} />
          )}
        </div>
        ...
        <div className="hidden sm:flex gap-4 opacity-90 text-[13px] font-medium">
          {["File", "Edit", "View", "Go", "Window", "Help"].map((menu) => (
            <div key={menu} className="relative">
              <span
                className={`cursor-default px-2 py-0.5 rounded hover:bg-white/10 ${
                  activeMenu === menu.toLowerCase() ? "bg-white/10" : ""
                }`}
              ...
              {activeMenu === menu.toLowerCase() && (
                <Dropdown items={menuItems[menu.toLowerCase()]} />
              )}
            </div>
          ))}
        </div>
    ...
    <div className="absolute top-7 left-0 w-48 bg-white/60 backdrop-blur-xl border border-white/20 shadow-xl rounded-lg py-1 flex flex-col z-[101]">
      {items.map((item, index) => (
        <div key={index} className="flex flex-col">
          <div className="px-4 py-1 text-sm text-black hover:bg-blue-500 hover:text-white cursor-default transition-colors">
            {item.label}
          </div>
